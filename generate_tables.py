...
class Tables():
    def __init__(self, discount=0.02, informal=0, filename='results/aggregate_results_imputed.csv'):
    ...
    def generate_table3(self):
        ...
        data3 = DALY.groupby('Region').sum().reset_index()
        data3['location'] = data3['Region']
        data3['daly2020'] = data3['2020'] / 1000000
        data3['daly2020_Ratio'] = data3['daly2020'] / data3['daly2020'].sum()
        data3['daly2050'] = data3['2050'] / 1000000
        data3['daly2050_Ratio'] = data3['daly2050'] / data3['daly2050'].sum()

        data4 = DALY.groupby('Income group').sum().reset_index()
        data4['location'] = data4['Income group']
        ...
        data_info = pd.concat([data1, data2])
        data_daly = pd.concat([data3, data4])
        data = pd.merge(data_info, data_daly, on='location')
        ...
        data[['location', 'GDP 2020', 'POP 2020', 'DALY 2020', 'DALY 2050']].to_csv('tables/Table3_informal%s_discount%s.csv'%(self.state['informal'], self.state['discount']), index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--informal', type=float, default=0) # or 0, 0.05, 0.11, 0.23
